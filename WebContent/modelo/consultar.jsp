...
<body>
	...
	<table>
		...
		<tbody>
		<% for(Modelo nModelo : listModelo) { %>
		<tr>
			<td><%= nModelo.getIdModelo() %></td>
			<td><%= nModelo.getDescricao() %></td>
			<td><%= nModelo.getMarca().getDescricao() %></td>
			<td>
				<a class="btn btn-delete" href="consultar.jsp?newAction=remove&codigo=<%= nModelo.getIdModelo() %>" onclick="return confirmarExclusao();">Excluir</a>
				<a href="manter.jsp?newAction=detail&codigo=<%= nModelo.getIdModelo() %>" class="btn btn-edit">Editar</a>
			</td>
		</tr>
		<% } %>		
		</tbody>
